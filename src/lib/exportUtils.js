...
async function fetchClubData(eventId, clubFull) {
  // 1. Fetch accreditations ONLY for this specific club
  const allAccs = await AccreditationsAPI.getByEventId(eventId, { club: clubFull }) || [];
  
  // 2. Fetch all attendance records for this event
  const allAttendance = await AttendanceAPI.getEventAttendance(eventId) || [];

  // 3. Fetch zones
  const { ZonesAPI } = await import("./storage");
  const zones = await ZonesAPI.getByEventId(eventId) || [];
  const permZones = zones.filter(z => z.settings?.scanMode === 'permanent' && !z.settings?.isHidden);

  // We already filtered server-side, but do a strict client-side check just in case of substring matches
  ...
  const formattedData = clubMembers.map((member, index) => {
    ...
    return {
      ...
      "Clearances": permZones.map(z => {
        const scan = memberAttendance.find(s => s.scanner_location === z.name || s.scanner_location?.startsWith(z.name));
        return `${z.name}: ${scan ? 'OK' : '...'}`;
      }).join(" | ") || "N/A"
    };
  });
...
async function generatePdfBlob(clubFull, clubData) {
  ...
  const headers = [["Sr#", "Name", "Club Name", "Category", "Status", "Attendance", "Clearances"]];
  const body = clubData.map(row => [
    row["Sr#"], 
    row["Name"], 
    row["Club Name"], 
    row["Category"], 
    row["Accreditation Status"], 
    row["Attendance"],
    row["Clearances"]
  ]);
