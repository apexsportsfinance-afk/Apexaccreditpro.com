...
export const generatePdfForAccreditation = async (accreditation, event, zones, pdfSize = "a6") => {
  ...
  try {
    ...
    const { EventCategoriesAPI } = await import("./storage");
    if (event?.id) {
      const [bg, cf, onlyFront, cats] = await Promise.all([
  ...
  const [frontCanvas, backCanvas] = await Promise.all([
    html2canvas(frontEl, { ...captureOpts, canvas: createCustomCanvas() }),
    backEl ? html2canvas(backEl, { ...captureOpts, canvas: createCustomCanvas() }) : Promise.resolve(null)
  ]);
