...
class AudioService {
    ...
    try {
      ...
      if (playPromise !== undefined) {
        playPromise.catch(error => {
          console.warn("Audio file playback failed, likely due to autoplay policy:", error);
        });
      }
    } catch (e) {
  ...
  speak(text, options = {}) {
    ...
    try {
      ...
      if (options.language) {
        preferredVoice = voices.find(v => v.lang === options.language) || 
                         voices.find(v => v.lang.startsWith(options.language.split('-')[0]));
      }
      ...
      if (!preferredVoice) {
        ...
        preferredVoice = femaleVoice || voices.find(v => v.lang.includes('en-GB') || v.lang.includes('en-US'));
      }
