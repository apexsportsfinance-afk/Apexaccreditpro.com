...
export const AttendanceAPI = {
   ...
   * @param {string} data.scanMode - 'daily' | 'permanent'
   * @returns {Object} result indicating success, duplicate, or error
   */
  recordScan: async ({ eventId, athleteId, clubName, scannerLocation, date, sessionId, zoneOnly, scanMode }) => {
    try {
      const now = new Date();
      ...
      let query = supabase
        ...
        .select("id, scan_count, scanner_location")
        .eq("event_id", eventId)
        .eq("athlete_id", athleteId);

      if (sessionId) {
        query = query.eq("session_id", sessionId);
      } else if (scanMode !== 'permanent') {
        // Daily mode (default) filters by date. Permanent mode ignores date.
      ...
      if (existing) {
        // Requirement: Append location if it's a new zone scan
        const currentLoc = existing.scanner_location || "";
        const newLoc = scannerLocation || "Main Entrance";
        
        let updatedLoc = currentLoc;
        const existingZones = currentLoc.split(",").map(z => z.trim());
        if (!existingZones.includes(newLoc)) {
          updatedLoc = currentLoc ? `${currentLoc}, ${newLoc}` : newLoc;
        }
        ...
        const { error: updateError } = await supabase
          ...
          .update({ 
            ...
            scanner_location: updatedLoc
          })
          .eq("id", existing.id);

        if (updateError) {
          console.error("[AttendanceAPI] Update Error:", updateError);
          throw updateError;
        }
        ...
        return {
          ...
          isNew: false,
          isNewLocation: updatedLoc !== currentLoc // TRUE if we added a new zone
        };
      }
      ...
      const { error: insertError } = await supabase
        ...
        .insert([{
          ...
          scan_count: 1,
          punctuality_status: 'UNASSIGNED' // Satisfy DB check constraint
        }]);

      if (insertError) {
        console.error("[AttendanceAPI] Critical RecordScan Failure:", insertError);
  ...
  /**
   * Retrieves the most recent scan for an athlete in a specific zone
   */
  getLatestZoneScan: async (eventId, athleteId) => {
    try {
      // DEBUG: Fetch last 10 logs globally to see what's in the DB
      const { data: allLogs } = await supabase
        .from("unified_scan_logs")
        .select("athlete_id, scan_mode")
        .order("created_at", { ascending: false })
        .limit(10);
      
      console.log("[AttendanceAPI] Global Last 10:", allLogs);

      // Direct query for this specific athlete's logs
      const { data, error } = await supabase
        .from("unified_scan_logs")
        .select("scan_mode, athlete_id, created_at")
        .eq("athlete_id", athleteId)
        .order("created_at", { ascending: false })
        .limit(1);
      ...
      // Add a special property to the result for debugging in the UI
      let debugStatus = "";
      if (lastLog) {
        debugStatus = `Found: ${lastLog.scan_mode}`;
      } else {
        const foundAny = allLogs?.some(l => l.athlete_id === athleteId);
        debugStatus = `NotFound (AnyMatch: ${foundAny})`;
      }

      const returnObj = (lastLog?.scan_mode === "zone_check_in" || lastLog?.scan_mode === "zone_check_out") ? lastLog : null;
      if (returnObj) returnObj.debugStatus = debugStatus;
      else return { debugStatus };

      return returnObj;
    } catch (err) {
      console.error("[AttendanceAPI] getLatestZoneScan error:", err);
      return null;
    }
  },
