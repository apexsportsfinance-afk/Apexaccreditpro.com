...
export const AttendanceAPI = {
  ...
  recordScan: async (scanData) => {
    ...
    try {
      ...
      if (existing) {
        // [NEW] If ignoreDuplicates is true, skip updating the database entirely
        if (ignoreDuplicates) {
          return {
            status: "success",
  ...
  getLatestZoneScan: async (eventId, athleteId) => {
    try {
      ...
      return (lastLog?.scan_mode === "zone_check_in" || lastLog?.scan_mode === "zone_check_out") ? lastLog : null;
    } catch (err) {
   ...
   */
  getScanLogs: async (limit = 100, offset = 0) => {
    try {
      const { data, error } = await supabase
        .from("unified_scan_logs")
        .select(`
          *,
          accreditations:athlete_id (id, first_name, last_name, club, badge_number, role, gender, nationality, custom_message, expires_at),
