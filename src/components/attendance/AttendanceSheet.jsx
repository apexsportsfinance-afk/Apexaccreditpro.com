...
export default function AttendanceSheet({ event, onBack }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const results = await Promise.allSettled([
        ...
        ZonesAPI.getByEventId(event.id)
      ]);
      ...
      const eventZones = results[5].status === 'fulfilled' ? results[5].value : [];
  ...
  const combinedData = useMemo(() => {
    ...
    return filteredAccs.map((person, idx) => {
      ...
      const permZones = zones.filter(z => z.settings?.scanMode === 'permanent');
      denominator = Math.max(manualGeneralSessions.length + permZones.length, 1);

      // Refined Logic: Count unique completions instead of total scan counts
      const clearedSessionIds = new Set(history.filter(r => r.session_id).map(r => r.session_id));
      const clearedZoneNames = new Set(history.map(r => r.scanner_location));

      const sessionNumerator = manualGeneralSessions.filter(s => clearedSessionIds.has(s.id)).length;
      const zoneNumerator = permZones.filter(z => 
        clearedZoneNames.has(z.name) || 
        Array.from(clearedZoneNames).some(loc => loc && loc.startsWith(z.name))
      ).length;

      numerator = sessionNumerator + zoneNumerator;
  ...
  }, [accreditations, attendanceRecords, athleteEventData, event, sessions, allAttendanceRecords, zones]);
