...
import { Download, FileSpreadsheet, FileText, Edit, CheckSquare, Square } from "lucide-react";
...
export default function BulkOperations({ 
  ...
  const handleBulkDownload = async () => {
    ...
    await bulkDownloadPDFs(selectedData, event, zones, "a6", (current, total) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 mb-4 p-4 bg-slate-800/50 rounded-xl border border-slate-700/50">
      ...
      {selectedRows.length > 0 && (
        <>
          <Button variant="secondary" size="sm" onClick={() => setShowEditModal(true)} icon={Edit}>
            Bulk Edit
          </Button>
