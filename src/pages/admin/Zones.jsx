...
import { Plus, Edit, Trash2, MapPin, AlertCircle, Copy, ShieldCheck, Clock, Volume2, Upload, Play, Trash } from "lucide-react";
...
export default function Zones() {
  ...
  const [formData, setFormData] = useState({
    ...
    description: "",
    settings: {
      accessMode: "general", // general | time_restricted | check_in_out
      scanMode: "daily", // daily | permanent
      timeSlots: [],
      isHidden: false,
      successMessage: "",
      voiceEnabled: true,
      voiceSettings: { volume: 1.0, language: "en-US" },
    ...
    if (zone) {
      ...
      setFormData({
        ...
        settings: { 
          ...
          voiceEnabled: zone.settings?.voiceEnabled ?? true,
          voiceSettings: zone.settings?.voiceSettings || { volume: 1.0, language: "en-US" },
          messages: zone.settings?.messages || { ...defaultMessages }
        }
      });
    } else {
      resetForm();
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex flex-col">
          ...
          {row.settings?.scanMode === "permanent" && (
        ...
        <form onSubmit={handleSubmit} noValidate className="p-6 space-y-4">
          ...
          <div className="pt-4 border-t border-slate-700/50">
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              <button
                type="button"
                onClick={() => setFormData(p => ({ ...p, settings: { ...p.settings, scanMode: "daily" } }))}
              ...
              >
                ...
                <span>Daily Scan Mode</span>
              </button>
              <button
                type="button"
                onClick={() => setFormData(p => ({ ...p, settings: { ...p.settings, scanMode: "permanent" } }))}
                className={`py-4 px-2 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all flex flex-col items-center gap-2 ${
                  formData.settings?.scanMode === "permanent"
