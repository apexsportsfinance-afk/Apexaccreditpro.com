...
export default function AuditLogView({ event }) {
  ...
  const filteredLogs = useMemo(() => {
    const filtered = logs.filter(log => {
      ...
      const role = (log.accreditations?.role || "").toLowerCase();
      const nationality = (log.accreditations?.nationality || "").toLowerCase();
      const customMsg = (log.accreditations?.custom_message || "").toLowerCase();
  ...
  const exportToExcel = async () => {
    try {
      ...
      const logExport = filteredLogs.map(log => ({
        ...
        ...(() => {
          try {
            const msg = log.accreditations?.custom_message;
            if (!msg) return {};
            const custom = typeof msg === 'string' ? JSON.parse(msg) : msg;
            const filtered = {};
            Object.keys(custom).forEach(key => {
              if (!key.toLowerCase().includes('url') && typeof custom[key] !== 'object') {
                filtered[key.charAt(0).toUpperCase() + key.slice(1).replace(/_/g, ' ')] = custom[key];
              }
            });
            return filtered;
          } catch (e) {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
        {/* Main Gate Access Widget */}
        <div className="relative group col-span-2 md:col-span-1 lg:col-span-1">
          <div className="absolute -inset-0.5 bg-gradient-to-b from-primary-500/10 to-ocean-500/10 rounded-3xl opacity-10 group-hover:opacity-20 transition duration-500 blur" />
          <div className="relative bg-gradient-to-b from-[#f2f1ec] to-[#dbdad2] border border-[#b5b4ad]/35 p-5 pt-6 rounded-[22px] h-full flex flex-col justify-between overflow-hidden shadow-[inset_0_1px_2px_rgba(255,255,255,0.85),_0_8px_24px_rgba(0,0,0,0.12)] group-hover:border-[#dfc58b]/60 group-hover:-translate-y-1 group-hover:shadow-[inset_0_1px_2px_rgba(255,255,255,0.85),_0_12px_36px_rgba(0,0,0,0.16)] transition-all duration-300">
            <div className="absolute top-0 inset-x-0 h-1 bg-[#dfc58b]" />
            <div>
            ...
            <div className="mt-4 pt-4 border-t border-[#b5b4ad]/25">
              <div className="flex justify-between items-center mb-2">
                <div className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-primary-500 shadow-[0_0_10px_rgba(59,130,246,0.6)] animate-pulse" />
                  <span className="text-[8px] font-black uppercase text-slate-800 tracking-wider">Occupancy</span>
                </div>
                <span className="text-[10px] font-bold text-slate-900 tracking-tight">{accreditations.length > 0 ? Math.round((totalUniqueScanned / accreditations.length) * 100) : 0}%</span>
              </div>
              <div className="h-1.5 w-full bg-[#c3c2bc]/40 rounded-full overflow-hidden">
                <motion.div initial={{ width: 0 }} animate={{ width: accreditations.length > 0 ? `${Math.min((totalUniqueScanned / accreditations.length) * 100, 100)}%` : '100%' }} className="h-full bg-primary-600 shadow-[0_0_8px_rgba(182,139,58,0.3)]" />
              </div>
            </div>
          </div>
        </div>
        {Object.entries(areaSummary).map(([area, data], idx) => {
          const occupancyPct = data.allocated > 0 ? Math.round((data.scanned.size / data.allocated) * 100) : 0;
          
          // Tactical Module State and Visual Parameters
          let cardBgClass = 'from-[#f5f4ef] to-[#dbdad1]';
          let borderClass = 'border-[#b5b4ad]/35';
          let hoverBorderClass = 'group-hover:border-[#dfc58b]/60';
          let topBarClass = 'bg-[#10b981]'; // Emerald operational default
          let statusLabel = `${occupancyPct}% ACTIVE`;
          let statusColorClass = 'bg-emerald-500';
          let statusGlowClass = 'shadow-[0_0_10px_rgba(16,185,129,0.6)]';
          ...
          if (data.allocated === 0) {
            // Live Scanner Station Mode
            cardBgClass = 'from-[#f7f6f1] to-[#dddcd3]';
            borderClass = 'border-[#b5b4ad]/40';
            hoverBorderClass = 'group-hover:border-cyan-500/50';
            topBarClass = 'bg-cyan-500';
            statusLabel = 'LIVE SCAN';
            statusColorClass = 'bg-cyan-500';
            statusGlowClass = 'shadow-[0_0_10px_rgba(6,182,212,0.6)]';
            barColorClass = 'bg-cyan-500';
            barGlowClass = 'shadow-[0_0_8px_rgba(6,182,212,0.3)]';
          } else if (data.scanned.size === 0) {
            // Low-Energy Standby / Monitoring Idle Mode
            cardBgClass = 'from-[#ecebe5] to-[#d3d2cb]';
            borderClass = 'border-[#b5b4ad]/25';
            hoverBorderClass = 'group-hover:border-slate-400/40';
            topBarClass = 'bg-slate-400/50';
            statusLabel = 'STANDBY';
            statusColorClass = 'bg-slate-400';
            statusGlowClass = 'shadow-[0_0_6px_rgba(148,163,184,0.4)]';
            ...
            cardBgClass = 'from-[#fbf2f0] to-[#e0cac4]';
            borderClass = 'border-rose-400/45';
            hoverBorderClass = 'group-hover:border-rose-500/80';
            topBarClass = 'bg-rose-500';
            statusLabel = `${occupancyPct}% CRITICAL`;
            statusColorClass = 'bg-rose-500';
            statusGlowClass = 'shadow-[0_0_12px_rgba(244,63,94,0.8)]';
            barColorClass = 'bg-rose-500';
            barGlowClass = 'shadow-[0_0_10px_rgba(244,63,94,0.4)]';
          } else if (occupancyPct >= 70) {
            // Moderate Load Warning Mode
            cardBgClass = 'from-[#f8f6f0] to-[#dbd7c3]';
            borderClass = 'border-amber-400/40';
            hoverBorderClass = 'group-hover:border-amber-500/70';
            topBarClass = 'bg-amber-500';
            statusLabel = `${occupancyPct}% WARNING`;
            statusColorClass = 'bg-amber-500';
            statusGlowClass = 'shadow-[0_0_12px_rgba(245,158,11,0.7)]';
            barColorClass = 'bg-amber-500';
            barGlowClass = 'shadow-[0_0_10px_rgba(245,158,11,0.4)]';
          }

          return (
            <motion.div key={area} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className="relative group">
              <div className="absolute -inset-0.5 bg-gradient-to-b from-slate-500/5 to-slate-600/5 rounded-3xl opacity-5 group-hover:opacity-15 transition duration-500 blur" />
              <div className={`relative bg-gradient-to-b ${cardBgClass} ${borderClass} border p-5 pt-6 rounded-[22px] h-full flex flex-col justify-between overflow-hidden ${hoverBorderClass} shadow-[inset_0_1px_2px_rgba(255,255,255,0.85),_0_8px_24px_rgba(0,0,0,0.12)] group-hover:-translate-y-1 group-hover:shadow-[inset_0_1px_2px_rgba(255,255,255,0.85),_0_12px_36px_rgba(0,0,0,0.16)] transition-all duration-300`}>
                
                {/* Top Status Metallic Reveal Accent Bar */}
                ...
                <div className="mt-auto">
                  <div className="flex justify-between items-center mb-2">
                    <div className="flex items-center gap-1.5">
                      <span className={`w-2 h-2 rounded-full ${statusColorClass} ${statusGlowClass} ${pulseClass}`} />
                      <span className="text-[8px] font-black uppercase text-slate-800 tracking-wider">Occupancy</span>
                    </div>
                    <span className={`text-[10px] font-bold ${textDimClass} tracking-tight`}>
                      {statusLabel}
                    </span>
                  </div>
                  <div className="h-1.5 w-full bg-[#c3c2bc]/30 rounded-full overflow-hidden">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: data.allocated > 0 ? `${Math.min((data.scanned.size / data.allocated) * 100, 100)}%` : '100%' }}
                      className={`h-full ${barColorClass} ${barGlowClass}`}
                    />
                  </div>
                </div>
              </div>
                        ...
                        <motion.tr key={log.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(idx * 0.02, 0.4) }} className="hover:bg-white/[0.03] transition-all group/row border-l-4 border-l-transparent hover:border-l-primary-500">
                          ...
                          <td className="px-10 py-7">
                            <div className="flex flex-col gap-0.5">
                              ...
                              <p className="text-[9px] text-primary-500/50 font-black uppercase tracking-widest">
                                {log.accreditations?.role || 'External'} • {log.accreditations?.club || 'No Club'} • {log.accreditations?.nationality || 'N/A'}
                                {(() => {
                                  try {
                                    const msg = log.accreditations?.custom_message;
                                    if (!msg) return "";
                                    const custom = typeof msg === 'string' ? JSON.parse(msg) : msg;
                                    const topFields = Object.entries(custom)
                                      .filter(([k, v]) => !k.toLowerCase().includes('url') && typeof v !== 'object')
                                      .slice(0, 2)
                                      .map(([k, v]) => `${v}`)
                                      .join(' • ');
                                    return topFields ? ` • ${topFields}` : "";
                                  } catch (e) { return ""; }
                                })()}
                              </p>
                            </div>
