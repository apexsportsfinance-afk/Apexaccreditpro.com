...
export default function AuditLog() {
  ...
  useEffect(() => {
    ...
    else loadScannerLogs(pageOffset);
  }, [activeTab, selectedEventId, pageOffset]);

  const loadLogs = async (offset) => {
    if (offset === 0) setLoading(true);
    else setIsLoadingMore(true);
    
    try {
      const limit = 100;
      const data = await AuditAPI.getRecent(limit, offset);
  ...
  const loadScannerLogs = async (offset) => {
    if (offset === 0) setLoading(true);
    else setIsLoadingMore(true);
    ...
    try {
      ...
      const data = selectedEventId 
        ? await AttendanceAPI.getScanLogsByEvent(selectedEventId, limit, offset)
        : await AttendanceAPI.getScanLogs(limit, offset);
      
      const newLogs = data || [];
      if (offset === 0) setScannerLogs(newLogs);
      else setScannerLogs(prev => [...prev, ...newLogs]);
    ...
    <div id="auditlog_page" className="space-y-6">
      ...
      <Card className="border-border ring-0 shadow-2xl">
        ...
        <CardContent className="p-0">
          ...
          {!loading && filteredLogs.length > 0 && hasMore && !search && (
    ...
    <table className="w-full text-left border-separate border-spacing-0">
      <thead>
        <tr className="bg-base-alt">
          ...
          <th className="py-4 px-6 text-[10px] font-black text-muted uppercase tracking-[0.2em] border-b border-border hidden sm:table-cell">Club Name</th>
          <th className="py-4 px-6 text-[10px] font-black text-muted uppercase tracking-[0.2em] border-b border-border hidden md:table-cell">Role</th>
        ...
        {sortedLogs.map((log, idx) => {
          ...
          return (
            ...
            >
              ...
              <td className="py-4 px-6 border-b border-border/30 group-hover:border-primary/20 hidden sm:table-cell">
                <span className="text-[10px] font-bold text-muted uppercase group-hover:text-main">
                  {club}
                </span>
              </td>
              <td className="py-4 px-6 border-b border-border/30 group-hover:border-primary/20 hidden md:table-cell">
      ...
      <tbody className="divide-y divide-border/30">
        {data.map((row, idx) => (
          ...
          >
            ...
            <td className="py-4 px-6 border-b border-border/30 hidden sm:table-cell">
              <span className="text-[10px] font-bold text-muted uppercase">{row.club}</span>
            </td>
            <td className="py-4 px-6 border-b border-border/30 hidden md:table-cell">
