        ...
        if (isZoneLocked) {
          ...
          if (!hasAccess) {
            ...
            if (isClearanceZone) {
              ...
              audioService.playSuccessEntry();
              const customMsg = activeZoneConfig?.settings?.successMessage || 
                               eventSettings?.voice_success_message || 
                               "Thank you.";
              audioService.speak(customMsg);
          ...
          if (activeZoneConfig?.settings?.accessMode === "time_restricted") {
            ...
            if (isTrulyNew) {
              audioService.playSuccessEntry();
              // DYNAMIC VOICE: Check Zone Settings > Event Settings > Fallback
              const customMsg = activeZoneConfig?.settings?.successMessage || 
                               eventSettings?.voice_success_message || 
                               "Thank you.";
              audioService.speak(customMsg);
              ...
              <div className={`absolute -bottom-2 left-1/2 -translate-x-1/2 px-6 py-2 rounded-full border-2 shadow-2xl z-20 whitespace-nowrap ${isFlagged ? 'bg-red-600 border-white' : (result.isPermanentCompletion ? 'bg-emerald-600 border-white ring-4 ring-emerald-500/30' : 'bg-emerald-600 border-white')}`}>
                 <div className="flex flex-col items-center">
                   <span className="text-xs font-black text-white uppercase tracking-[0.2em]">
                     {isFlagged ? athlete.status : (result.isPermanentCompletion ? 'COMPLETED' : (result.message || 'ACCESS GRANTED'))}
                   </span>
