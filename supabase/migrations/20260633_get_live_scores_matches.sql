-- ==============================================================================
-- PHASE 1 (Live Score visual upgrade): expose team logo/flag info alongside
-- live_score_matches rows so the public QR live-scores widget and the Team
-- Portal schedule (both anon/authenticated, no direct RLS access to the full
-- `teams` table) can render team badges without any new grants on `teams`.
--
-- Mirrors the existing get_team_standings() SECURITY DEFINER pattern. Purely
-- additive: a new read-only RPC, no changes to existing tables or functions.
-- ==============================================================================
...
AS $$
  SELECT
    m.id, m.event_id, m.sport_id, m.match_title, m.team_a_id, m.team_b_id,
