...
// no redeploy needed.
async function getSenderConfig(brandName?: string): Promise<{ from: string; replyTo: string }> {
  // Layer-1 white-label: the org's display NAME overrides the sender name when
  // provided. The from ADDRESS stays the platform's verified mailbox (per-org
  // sending domains / DKIM are a later Layer-2 effort).
  try {
    ...
    if (raw) {
      ...
      if (cfg?.fromEmail) {
        return {
          from: `${brandName || cfg.fromName || "Apex Sports Accreditations"} <${cfg.fromEmail}>`,
...
function buildTemplateHtml(bodyText: string, headerTitle: string, headerColor: string, brand: string = "Apex Sports Accreditations"): string {
...
function buildRejectedHtml(params: { name: string, eventName: string, role: string, remarks: string, resubmitUrl: string }, brand: string = "Apex Sports Accreditations"): string {
...
function buildCustomHtml(name: string, bodyText: string, brand: string = "Apex Sports Accreditations"): string {
  const htmlParagraphs = bodyText.split("\n").map(line => {
    if (line.trim() === "") return `<br/>`;
    return `<p style="color:#e2e8f0;font-size:15px;line-height:1.6;margin:0 0 8px 0;">${line}</p>`;
  }).join("");

  return `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head><body style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif;background-color:#f1f5f9;margin:0;padding:0;"><div style="max-width:600px;margin:0 auto;padding:40px 20px;"><div style="background:#0f172a;border-radius:16px;overflow:hidden;"><div style="background:linear-gradient(135deg,#3b82f6,#2563eb);padding:32px;text-align:center;"><h1 style="color:white;margin:0;font-size:24px;">${brand}</h1></div><div style="padding:32px;">${htmlParagraphs}</div><div style="padding:24px 32px;border-top:1px solid #1e293b;text-align:center;"><p style="color:#475569;font-size:12px;margin:0;">This is an automated message from ${brand}</p></div></div></div></body></html>`;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { to, name, eventName, eventLocation, eventDates, role, accreditationId, badgeNumber, zoneCode, reportingTimes, remarks, resubmitUrl, type, customSubject, customBody, pdfBase64, pdfFileName, templateBody, templateSubject, brandName } = body;

    // Per-org display brand (Layer 1). Absent/empty => keep Apex wording.
    const brand: string = (brandName && String(brandName).trim()) || "Apex Sports Accreditations";
    ...
    if (type === "custom") {
      ...
      htmlBody = buildCustomHtml(name || "Participant", customBody || "No message content.", brand);
      if (pdfBase64) {
      ...
      if (templateBody) {
        ...
        htmlBody = buildTemplateHtml(templateBody, eventName, "#3b82f6,#2563eb", brand);
      } else {
        subject = `Accreditation Approved - ${eventName}`;
        htmlBody = buildApprovedHtml({ name, eventName, eventLocation: eventLocation || "", eventDates: eventDates || "", role: role || "", accreditationId: accreditationId || "", badgeNumber: badgeNumber || "", zoneCode: zoneCode || "", reportingTimes: reportingTimes || "" }, brand);
      }
      ...
      if (templateBody) {
        ...
        htmlBody = buildTemplateHtml(templateBody, eventName, "#ef4444,#dc2626", brand);
      } else {
        subject = `Accreditation Rejected - ${eventName}`;
        htmlBody = buildRejectedHtml({ name, eventName, role: role || "", remarks: remarks || "", resubmitUrl: resubmitUrl || "" }, brand);
      }
    } else {
    ...
    const { from: senderFrom, replyTo: senderReplyTo } = await getSenderConfig(brand !== "Apex Sports Accreditations" ? brand : undefined);
